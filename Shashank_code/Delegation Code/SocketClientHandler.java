...
public class SocketClientHandler implements Runnable {
  ...
  public void run() {
   ...
   private void readResponse() throws IOException, InterruptedException {
	...
	//=============ADDING THESE NEW VARIABLES TO CAPTURE ALL DATA FOR DELEGATION============
	String ownerID;
	String clientID;
	String documentName;
	String permissions;
	...
	while ((userInput = stdIn.readLine()) != null) {
		...
		//===============ADDING THIS EXTRA ELSE-IF BLOCK TO CONSIDER PUTPERMISSIONS CALL FOR DELEGATION=================
		else if(userInput.equals("PUTPERMISSIONS"))
		{
			...
			days = Integer.parseInt(stdIn.readLine());
	...
	//=================NEW PUT METHOD FOR DELEGATION===============================================
	
	public void putPermissions(String OwnerID, String ClientID, String permissions, String delegate, String documentName, int days, BufferedReader stdIn) throws IOException
	{
		String userInput;
		FileOutputStream fos = null;
		try
		{
			...
			ByteBuffer buffer = ByteBuffer.allocate(view.size(ClientID));
			view.read(ClientID, buffer);
			buffer.flip();	
			String value = Charset.defaultCharset().decode(buffer).toString();
			...
			if((documentOwner.equals(OwnerID)) || (value.contains("Y"))){
				
				//THIS SETS THE KEY - VALUE PAIR AS "CLIENTID : PERMISSIONS,DELEGATE,DAYS"
				view.write(ClientID, Charset.defaultCharset().encode(permissions + "," + delegate + "," + days));
